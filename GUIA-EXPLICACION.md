...
Mi ejecución real fue así: cloné mi repo en una máquina Ubuntu con Docker, instalé Terraform a mano porque venía vacío, corrí `init` y `validate`, y al aplicar me topé con dos cosas que resolví. Primera: mi variable traía el socket de Windows y allá es Linux, así que apliqué con `-var='docker_host=unix:///var/run/docker.sock'`. Segunda: mi provider Docker traía un cliente viejo (API 1.41) y el daemon exigía mínimo 1.44, así que subí el provider a la versión 3.9 en mi `versions.tf`. Con eso escribí `yes`, mi `apply` terminó en `Apply complete! Resources: 2 added` y mi `curl http://localhost:8080` trajo mi página con el `sitio-portafolio OK`. Todo eso junto —mis supuestos, mi código, mis costos, mi reversión, mi justificación y mi evidencia— es mi entrega del taller.
